...
import RentOrBuyGuide from "./guide.js";
...
describe("rent-or-buy guide", () => {
  it("has a question, an explanation and typical values for every input", () => {
    expect(Wizard.validateGuide(Calc, RentOrBuyGuide)).toBe(true);
  });

  for (const mode of ["live", "let"]) {
    it(`produces a complete answer screen in ${mode} mode`, () => {
      Calc.resetToDefaults();
      Calc.mode = mode;
      const out = RentOrBuyGuide.outcome(Calc, Calc.simulate());
  ...
  it("names every step once and keeps each section's questions together", () => {
    const ids = RentOrBuyGuide.steps.map((s) => s.id);
    expect(new Set(ids).size).toBe(ids.length);

    const seenSections = [];
    RentOrBuyGuide.steps.forEach((s) => {

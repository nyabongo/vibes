...
import BrickByBrickGuide from "./guide.js";
...
describe("brick-by-brick guide", () => {
  it("has a question, an explanation and typical values for every input", () => {
    expect(Wizard.validateGuide(Brick, BrickByBrickGuide)).toBe(true);
  });

  for (const mode of ["asyougo", "savefirst"]) {
    it(`produces a complete answer screen in ${mode} mode`, () => {
      Brick.resetToDefaults();
      Brick.mode = mode;
      const out = BrickByBrickGuide.outcome(Brick, Brick.simulate());
  ...
  it("names every step once and keeps each section's questions together", () => {
    const ids = BrickByBrickGuide.steps.map((s) => s.id);
    expect(new Set(ids).size).toBe(ids.length);

    const seenSections = [];
    BrickByBrickGuide.steps.forEach((s) => {

...
Model.mode = DEFAULT_MODE; // "gross" | "net" — which rental tax regime applies
Model.cur  = { code:"KES", sym:"KSh", rate:1 };
Model.suppressPersist = false;
...
Model.DEFAULTS = {};
Object.keys(Model.V).forEach(function(k){ Model.DEFAULTS[k] = Model.V[k]; });
...
Model.applyCurrency = function(code){
  for(var i=0;i<Model.CURRENCIES.length;i++){
    if(Model.CURRENCIES[i].code===code){
      Model.cur = { code:Model.CURRENCIES[i].code, sym:Model.CURRENCIES[i].sym, rate:Model.CURRENCIES[i].rate };
      return true;
    }
  }
  return false;
...
Model.resetToDefaults = function(){
  Object.keys(Model.DEFAULTS).forEach(function(k){ Model.V[k] = Model.DEFAULTS[k]; });
  Model.mode = DEFAULT_MODE;
  Model.applyCurrency(DEFAULT_CUR_CODE);
};
...
Model.PARAM_MAP = PARAM_MAP;
Model.PARAM_MAP_REV = PARAM_MAP_REV;
Model.STORAGE_KEY = STORAGE_KEY;

Model.buildQueryString = function(){
  var params = new URLSearchParams();
  Object.keys(PARAM_MAP).forEach(function(k){
    if(Math.abs(Model.V[k]-Model.DEFAULTS[k]) > 1e-9) params.set(PARAM_MAP[k], Model.V[k]);
  });
  if(Model.mode !== DEFAULT_MODE) params.set("m", Model.mode);
  if(Model.cur.code !== DEFAULT_CUR_CODE) params.set("c", Model.cur.code);
  return params.toString();
};

Model.updateURL = function(){
  var qs = Model.buildQueryString();
  if(typeof window !== "undefined" && window.history && window.location){
    history.replaceState(null, "", location.pathname + (qs?("?"+qs):"") + location.hash);
  }
  if(!Model.suppressPersist){
    try{ localStorage.setItem(STORAGE_KEY, JSON.stringify({ V:Model.V, mode:Model.mode, cur:Model.cur.code })); }catch(e){}
  }
};

Model.loadFromStorage = function(raw){
  try{
    if(raw === undefined) raw = localStorage.getItem(STORAGE_KEY);
    if(!raw) return;
    var data = JSON.parse(raw);
    if(data.V) Object.keys(data.V).forEach(function(k){
      if(Object.prototype.hasOwnProperty.call(Model.V, k) && typeof data.V[k]==="number" && isFinite(data.V[k])) Model.V[k] = Model.clampToField(k, data.V[k]);
    });
    if(data.mode==="gross"||data.mode==="net") Model.mode = data.mode;
    if(data.cur) Model.applyCurrency(data.cur);
  }catch(e){}
};

Model.clampToField = function(k, num){
  var f = Model.FIELD_BY_KEY[k];
  if(!f) return num;
...
Model.paramKey = function(key){
  return Object.prototype.hasOwnProperty.call(PARAM_MAP_REV, key) ? PARAM_MAP_REV[key] : undefined;
};

Model.hasScenarioParams = function(search){
  if(search === undefined) search = (typeof location !== "undefined" ? location.search : "");
  var found = false;
  new URLSearchParams(search).forEach(function(val, key){
    if(key==="m" || key==="c" || Model.paramKey(key)) found = true;
  });
  return found;
};

Model.loadFromURL = function(search){
  if(search === undefined) search = (typeof location !== "undefined" ? location.search : "");
  var params = new URLSearchParams(search);
  params.forEach(function(val, key){
    if(key==="m"){ Model.mode = (val==="net") ? "net" : "gross"; return; }
    if(key==="c"){ Model.applyCurrency(val); return; }
    var k = Model.paramKey(key);
    if(k){ var num = parseFloat(val); if(isFinite(num)) Model.V[k] = Model.clampToField(k, num); }
  });
};
...
Model.FIELD_BY_KEY = {};
Object.keys(Model.FIELDS).forEach(function(id){
  Model.FIELDS[id].forEach(function(f){ Model.FIELD_BY_KEY[f.k] = f; });
});

/* ===================== formatting ===================== */
Model.fmt = function(n){
  var x = n * Model.cur.rate;
  return (x<0?"−":"") + Model.cur.sym + Math.round(Math.abs(x)).toLocaleString("en-US");
};
Model.fmtC = function(n){ // compact, for axes
  var x = n * Model.cur.rate, s = x<0?"−":"", a = Math.abs(x);
  if(a>=1e9) return s+Model.cur.sym+(a/1e9).toFixed(1).replace(/\.0$/,"")+"B";
  if(a>=1e6) return s+Model.cur.sym+(a/1e6).toFixed(1).replace(/\.0$/,"")+"M";
  if(a>=1e3) return s+Model.cur.sym+Math.round(a/1e3)+"k";
  return s+Model.cur.sym+Math.round(a);
};
Model.pctS = function(x){ return (Math.round(x*10)/10) + "%"; };
...
Model.simulate = function(o){
  o = o || {};
  var V = Model.V;
  var mode     = o.mode     !== undefined ? o.mode     : Model.mode;
  ...
  var gInv  = mrate(Model.netInvestReturn(o));
...
Model.solve = function(key, lo, hi){
  function f(x){ var o={}; o[key]=x; var s=Model.simulate(o); return s.finalBuild - s.finalInvest; }
  var a=f(lo), b=f(hi);
...
Model.irr = function(cashflows){
  var cf = cashflows || Model.simulate().cashflows;
